...
조사 당시 query 응답에는 latest attempt ID·source hash가 모두 들어 있지 않았다. MFG-12는 query에 latest attempt ID를
추가하고 설명 응답에서 검증한 source hash를 결합했다. 실제 schema는 파일 계약이 소유하며 LLM 전송 허용을 뜻하지 않는다.
...
자유 대화용 route·schema·오류 코드·지연/비용 숫자는 별도 계약과 테스트에서 닫는다. MFG-12의 규칙 기반 explanation API와 혼동하지 않는다.
...
2. **MFG-12의 첫 로컬 Slice 완료:** 하단 진입 → 맥락 고정 → 실제 서버 근거 → 선택 질문의 규칙 기반 설명 →
   같은 버전 근거 보기까지 구현했다. [직접 검토·검증](../VERIFICATION.md#guided-explanation-verification)이 근거다.
   수치는 실제 query 결과이며 자유 대화나 AI 품질의 완료 증거는 아니다.
...
최초 조사 완료의 근거는 출처·화면 비교와 기존 code/API 대조였다. 이후 역할 방향을 수용해 MFG-12의 선택 질문 기능을
구현·검증했다. 실제 AI 평가·독립 사용자의 사용 성공·외부 AI 호출·새 Release·배포는 수행하지 않았다.
