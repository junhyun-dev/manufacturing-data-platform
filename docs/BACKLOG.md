...
**공개 조사·현재 코드 대조 완료 / 역할 선택 대기 / 구현 미승인.** 작성자는 제품 우측 하단의 대화 기능 조사를 요청했다.
[실제 사례·화면 추천·Data/API·안전 검증안](research/review-assistant.md)이 조사와 미수용 설계를 소유한다.
...
- 다음 한 결정: 현재 결과 설명형·자유 분석형·실행형 중 첫 역할을 작성자가 선택한다. 추천은 현재 결과 설명형이다.
