...
**로컬 구현·검증 완료 — 2026-09-09.** 기준 `defbacd452257ff626ffa1888005fbf7135940e9`.
...
- 결과: [검증 기록](VERIFICATION.md#guided-explanation-verification). 전체 255 pass / 17 skip, 실제 HTTP와
  full/sample 설명 18/12개 시나리오, 기존 화면 11/7개 시나리오를 통과했다. 모바일 포커스·시간 초과의 늦은 응답과
  삭제 확인창 Escape 충돌을 직접 검토에서 보정했다. UI 숫자·원본·버전은 실제 query/ZIP과 일치한다.
- 후속 근거: 실제 검토자가 이전 결과의 출처를 찾는지, 기존 화면보다 설명이 도움이 되는지 관찰한다.
  모바일에서 근거와 설명의 읽기 순서·숫자 표시 이해도도 확인한다. 자유 입력은 반복해서 나오지만 세 질문으로
  해결되지 않는 질문을 모은 뒤 MFG-10의 데이터·비용·평가 계약과 함께 판단한다.
